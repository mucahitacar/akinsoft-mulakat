...
1,8,8,2,6,7,0,4,2,8,2,5,2,4,8,3,6,0,0,8,2,3,2,5,7,5,3,0,4,2,0,7,5,2,9,6,3,4,5,0 };
    ...
    int index = 0;
    int carpim = 1;
    int dizi_boyutu = 1000;
    int ardisik;
    
        /*cout << "Kac elemana kadar bakilsin? (Max 1000 giriniz)";
        cin >> dizi_boyutu;
        cout << "Kac ardisik elemanın carpimina bakilsin? ";
        cin >> ardisik;*/
        if (dizi_boyutu <= 1000)
        {
            for (int j = 0; j < dizi_boyutu; j++) {
                ...
                if (carpim > max) {
                    max = carpim;
                    max_baslangic = index - 11;
                    max_bitis = index + 1;
                    carpim = 1;
                }
                carpim = 1;
            }
            cout << "En yuksek carpim sonucu:" << max << endl;
            cout << "Baslangic konumu:" << max_baslangic << endl;
            cout << "Bitis konumu:" << max_bitis << endl;
            cout << "-------------" << endl;
            cout << "UYARI: Konumlar dizinin elemanlarini sayma sayilarindan basladigi(1) dusunulerek belirlenmistir cunku verdiginiz ornekte de 365 degil 364ten basliyor ben de 1 fazlasini ekleyerek konumlari yazdim!" << endl;
            //soru_2();
        }
